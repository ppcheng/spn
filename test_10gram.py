...
word_vectors = gensim.models.KeyedVectors.load_word2vec_format(glove_100d)
...
with open(testing_data, 'r') as data:
    ...
    for i, row in enumerate(reader):
        ...
        if len(question) < 10:
            question = ['<pad>'] * (10-len(question)) + question
        else:
            if len(question[pos+1:]) < 4:
                question = question[pos-5-(4-len(question[pos+1:])):]
            elif len(question[:pos]) < 5:
                question = question[:pos+5+(5-len(question[:pos]))]
            else:
                question = question[pos-5:pos+5]
        print i+1, row['question'], question
    ...
    sent_vec = np.reshape(sent_vec, (-1, 1000))
    np.savetxt('./data/test_matrix_10gram_100d', sent_vec, fmt='%.4f', delimiter=',')